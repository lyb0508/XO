...
- 默认离线套件 165 项收集：163 通过，2 项 live 测试默认跳过；连续三次稳定（约 2.6–3.1 秒）。新增覆盖：元数据完整性、设备绑定、top_k/阈值、确定性排序、校准样例可分性、边界契约（空查询/top_k 越界/min_score>1/零向量/分数并列）、会话隔离/截断/清理、台账白名单/容错/limit、图级 RAG 引用、严格阈值空结果不失败、批准写台账而拒绝不写、planner 记忆注入开关。
- 第一轮独立只读审查（PARTIAL）：4 项 P2 全部关闭（builder 注解缺导入实证修复、`--session` 空白值规范化防止报告与副作用脱节、ingest 先嵌入后入库保证原子性、"构建不联网"声明更正为"摄取时联网"并在 CLI 转译为清晰错误）；5 项 P3 一并修复（AGENT_VERSION 更新、ledger 死代码 try 移除、settings 标注校准参考区间、零向量查询拒收、store 边界契约测试固化）。延期 4 项 P3 已记录：already_executed 补偿性台账补写（审计语义设计，随真实存储阶段处理）、ledger 直调防御性枚举校验、manual_query 最小长度两处语义统一、转换层 version fallback 显式报错（涉及阶段1共享路径）。
