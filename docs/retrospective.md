...
| 最终质量指标 | 未达成 | 三项 live 指标 0.81 / 0.727 / 0.84，差距已量化并有改进排序 |
...
| 最终目标指标 | 目标 | 当前 live 基线 | 判定 |
| --- | --- | --- | --- |
| 关键结构化输出有效率 | 100% | final_contract 0.90 | ✗ |
| 高风险动作人工审批覆盖率 | 100% | 所有副作用动作均经 interrupt 门禁 | ✓ |
| 工具选择正确率 | ≥90% | 0.81 | ✗ |
| 证据不足追问/拒答正确率 | ≥90% | 0.727（strict 口径） | ✗ |
| 关键轨迹稳定率 | ≥85% | 0.84 | ✗（差 1pp） |

19 个失败案例根因已定位并按收益排序：① 计划 JSON 解析失败 ×5（最大单一来源，可用重试/降级策略回收约 10% 失败）；② device_info 场景过度保守 4/4；③ insufficient 场景自作假设 4/6；④ manual_only 误澄清 3/4；⑤ multi_tool 子集偏差 ×2。改进杠杆：PLAN_PROMPT 针对性引导、更强规划模型或 function_calling 迁移。

诚实声明：以上为 qwen2.5:7b 单模型单轮基线；DeepSeek provider、--upload 远端上传、多轮会话与审批交互的轨迹评测均未验证。
