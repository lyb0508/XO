...
def test_cli_prints_exactly_one_report_json_to_stdout_and_passes_structured_method(monkeypatch, capsys) -> None:
    ...
    def build_stub(model, limits, structured_output_method):
        received["method"] = structured_output_method
        return object()

    monkeypatch.setattr(cli, "build_diagnostic_agent", build_stub)
    monkeypatch.setattr(cli, "run_diagnosis", lambda *args, **kwargs: _report())
...
@pytest.mark.parametrize("argv", [[], ["--question", "x", "--unknown"]])
